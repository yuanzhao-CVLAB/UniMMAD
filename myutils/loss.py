...
class CosLoss(nn.Module):
    ...
    def forward(self, input1, input2,mask=None):
        ...
        for in1, in2 in zip(input1, input2):
            if mask is None:
                loss += (1 - self.cos_sim(in1.contiguous(), in2.contiguous())).mean()
            else:
                cur_mask = (F.interpolate(mask,size=in2.shape[-2:]) == 0)
                loss += (1 - self.cos_sim(in1.contiguous(), in2.contiguous()))[cur_mask.squeeze(1)].mean()
